...
function Actions(user) {
	...
	this.create = async (data) => {
		...
		if (res.status === 201) {
			...
			const professionalId =
				res?.data?.included[0]?.relationships?.professional?.data.id;
	...
	this.search = async ({ queryKey }) => {
		const queryPath =
			'q[field_or_user_first_name_or_user_last_name_or_services_title_cont]';
		const [_, keyword] = queryKey;
		const { data } = Axios.get(path + `?${queryPath}=${keyword}`, config);
		return data;
	};
}
